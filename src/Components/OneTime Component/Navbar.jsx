...
return (
  <>
    ...
    <div className= {`menu absolute w-[300px] md:w-[450px] bg-[#FAF9F8] top-0 ${menuOpen ? 'left-0' : '-left-[100%]'} px-5 pt-5 overflow-auto`} >
      ...
      {data.map((item, index) => (
        <div key={index} className="h-16 overflow-hidden hover:h-full">
          ...
          <NavLink  onClick={handleMenuToggle} to={item.women} className="font-1 text-sm text-black h-16 px-10 capitalize tracking-wider flex items-center hover:text-red-400" > <span className=' block h-12 w-12 rounded-full overflow-hidden mr-3 '><img className=' object-cover scale-125' src="/img/women.jpeg" alt="/" /></span> Women  
          </NavLink>

          <NavLink onClick={handleMenuToggle} to={item.men} className="font-1 text-sm text-black h-16 px-10 capitalize tracking-wider flex items-center hover:text-red-400" > <span className=' block h-12 w-12 rounded-full overflow-hidden mr-3 '><img className=' object-center' src="/img/men.jpeg" alt="/" /></span> Men </NavLink>

          {item.kids.length >   0 ? (<NavLink onClick={handleMenuToggle} to={item.kids}   className="font-1 text-sm text-black h-16 px-10 capitalize tracking-wider flex items-center hover:text-red-400"> <span className="block h-12 w-12 rounded-full overflow-hidden mr-3"><img className=' object-center scale-125' src="/img/kids.jpg" alt="/" /></span> kids </NavLink>) : ("")}

          {item.sports.length > 0 ? (<NavLink onClick={handleMenuToggle} to={item.sports} className="font-1 text-sm text-black h-16 px-10 capitalize tracking-wider flex items-center hover:text-red-400"> <span className="block h-12 w-12 rounded-full overflow-hidden mr-3"><img className="object-center" src="/img/sports.jpeg" alt="/" /></span> Sports </NavLink>) : ("")}
